...
router.post("/text/:id/:userId/:reviewId/:venueId", async (req, res) => {
  ...
  try {
    const updatedReview = await resData.updateCommentText(
      id,
      userId,
      reviewId,
      venueId,
    ...
    const venueReviews = await resData.getAllCommentsByReviewId(
      reviewId,
      venueId
    );
    ...
    for (let i = 0; i < AllComments.length; i++) {
      AllComments[i].reviewerId = getUser.firstName.concat(
        " ",
        getUser.lastName
      );
...
router.get(
  "/updatecomment/:id/:userId/:reviewId/:venueId",
  async (req, res) => {
...
router.post("/:userId/:reviewId/:venueId", async (req, res) => {
  ...
  try {
    const postComment = await resData.addComment(
      userId,
      reviewId,
      venueId,
      commentText
